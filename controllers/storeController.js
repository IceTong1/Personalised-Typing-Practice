...
router.post('/buy', requireLogin, (req, res) => {
    ...
    if (!itemId || !storeItems[itemId]) {
        return res
            .status(400)
            .json({ success: false, message: 'Invalid item selected.' });
    }
    ...
    if (!currentUserDetails) {
        ...
        console.error(
            `Could not fetch details for logged-in user ID ${userId}`
        );
        return res
            .status(500)
            .json({ success: false, message: 'Could not verify user data.' });
    }
    ...
    try {
        // --- Add logging ---
        console.log(
            `Attempting purchase: User ID = ${userId}, Item ID = ${itemId}, Price = ${item.price}, User Coins = ${userCoins}`
        );
        // --- End logging ---
        ...
        if (!coinsDecremented) {
            ...
            console.warn(
                `Failed to decrement coins for user ${userId} buying item ${itemId} (likely insufficient funds or race condition).`
            );
            // Return 409 Conflict as the state wasn't valid for the operation
            return res.status(409).json({
                success: false,
                message: 'Insufficient balance or error during deduction.',
            });
        }
        ...
        if (!itemAdded) {
            ...
            console.error(
                `Failed to add item ${itemId} for user ${userId} after coin deduction.`
            );
            console.warn(
                `Attempting to refund ${item.price} coins to user ${userId}.`
            );
            db.increment_user_coins(userId, item.price); // Best effort refund
            return res.status(500).json({
                success: false,
                message:
                    'Failed to add item to inventory after purchase. Coins refunded.',
            });
        }
        ...
        console.error(
            `Unexpected error during purchase for user ${userId}, item ${itemId}:`,
            error
        );
...
router.get('/shelf', requireLogin, (req, res) => {
    ...
    try {
        ...
        console.error(
            `Error fetching or rendering shelf for user ${userId}:`,
            error
        );
