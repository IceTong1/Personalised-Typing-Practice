    ...
    const cancelDeleteBtn = document.getElementById('cancelDeleteBtn'); // Or rely on data-bs-dismiss
    ...
    function showTextDeleteConfirmation(textId, listItem) {
        ...
        const textTitle = listItem.querySelector('span').textContent.trim().split('\n')[0]; // Try to get title
        deleteConfirmationMessage.textContent = `Are you sure you want to delete the text "${textTitle || 'this text'}"?`;
        if (deleteModalInstance) {
            deleteModalInstance.show();
        } else {
            console.error("Delete confirmation modal instance not found! Cannot show confirmation.");
        }
    }
        ...
        try {
            ...
            if (response.ok || (response.status >= 300 && response.status < 400)) {
                 ...
                 if (response.redirected) {
                     ...
                     console.log(`Deletion request for ${url} resulted in a redirect to ${response.url}. Assuming server handled notification.`);
                     ...
                     if (contentType && contentType.includes('application/json')) {
                         ...
                         if (result.success) {
                             ...
                             console.error('Server indicated failure:', result.message);
                             alert(`Error: ${result.message || failureMessageBase}`);
                         }
                     } else {
                         // Response was OK but not JSON and not a redirect - might be unexpected HTML
                         console.warn(`Deletion request for ${url} succeeded with status ${response.status} but received unexpected content type: ${contentType}. Assuming success.`);
                         removeElementVisually(elementToRemove, successMessage);
                ...
                } catch (e) { /* Ignore JSON parsing error if status was already bad */ }
            ...
            console.error('Error message:', error.message);
             if (response) {
                 ...
                 } catch (headerErr) { /* Ignore */ }
             }
            alert(`An unexpected error occurred. Check browser console for details. Error: ${error.message}`);
        } finally {
    ...
    if (confirmDeleteBtn) {
        confirmDeleteBtn.addEventListener('click', handleGenericDeleteConfirmation); // Use the unified handler
    }
    ...
    document.body.addEventListener('click', (event) => {
        ...
        if (textDeleteButton) {
            ...
            if (textId && listItem) {
                ...
                console.error('Could not find text ID or list item for text deletion.');
            ...
            if (folderId && folderName) {
                showFolderDeleteConfirmation(folderId, folderName, folderDeleteButton);
            } else {
                // console.error("Folder ID or Name missing:", folderId, folderName); // DEBUG: Check data retrieval
                console.error('Could not find folder ID or name for folder deletion.');
